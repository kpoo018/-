...
형태소 분석기는 한자에 읽기가 여럿일 때 가장 흔한 것을 고르는데, 노래는 자주 그렇지 않다.
明日 를 아시타 대신 아스로, 行こう 를 이코오 대신 유코오로 부르는 식이다. 어느 쪽이 맞는지는
곡마다 다르므로 전역 사전으로는 풀 수 없다.

가사 줄을 **길게 누르면** 그 줄의 한자를 골라 읽기를 고칠 수 있다. 들리는 대로 한글로 적으면
되고(일본어 키보드가 필요 없다), 가타카나로 적어도 된다. 고친 읽기는 그 곡에만 저장되고
같은 곡 안의 같은 단어에 모두 적용된다. 되돌리기도 같은 자리에 있다.

교정은 캐시 위에 얹는 방식이라 원본은 그대로 남는다. 다시 분석만 하면 되므로 네트워크를
타지 않는다.

### 끄기

상단 스위치로 끈다. 알림의 **끄기** 버튼도 같은 일을 한다. 알림 리스너 서비스는 시스템이
붙잡고 있어 앱이 스스로 죽을 수 없으므로, 꺼지면 아무것도 그리지 않고 예약도 하지 않는
방식으로 멈춘다. 권한을 뺏을 필요가 없다.
...
권한이 회색으로 막혀 있으면 사이드로드 앱이라 그렇다.
설정 → 애플리케이션 → 가사 → 우측 상단 ⋮ → **제한된 설정 허용** 을 먼저 눌러야 한다.
...
- **안드로이드 앱은 컴파일·패키징·단위 테스트까지는 검증했지만 기기에서 돌려 보지는
  ...
  앱은 JVM 테스트 36 개가 통과한다 (Kuromoji 형태소 분석 포함).
